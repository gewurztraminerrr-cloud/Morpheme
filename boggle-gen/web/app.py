...
class Board(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    letters = db.Column(db.Text, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def handle_history():
    try:
        if request.method == 'POST':
            ...
            if letters:
                new_board = Board(letters=letters)
                db.session.add(new_board)
                db.session.commit()
                return jsonify({'success': True, 'id': new_board.id})
            return jsonify({'error': 'No letters provided'}), 400
        
        boards = Board.query.order_by(Board.created_at.desc()).limit(20).all()
        return jsonify([{'id': b.id, 'letters': b.letters, 'time': b.created_at.strftime('%H:%M:%S')} for b in boards])
