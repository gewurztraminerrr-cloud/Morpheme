...
def main():
    ...
    try:
        ...
        if idx == 0:
            ...
            print("\n--- Curling http://localhost:5001/ ---")
            child.sendline("curl -s -o /dev/null -w '%{http_code}' http://localhost:5001/")
            child.expect([r"\$", r"#", pexpect.EOF, pexpect.TIMEOUT], timeout=10)

            print("\n--- Curling http://localhost:5001/api/tournament/status ---")
            child.sendline("curl -s -i http://localhost:5001/api/tournament/status")
            child.expect([r"\$", r"#", pexpect.EOF, pexpect.TIMEOUT], timeout=10)

            print("\n--- Curling http://localhost:5001/api/lobby-stats ---")
            child.sendline("curl -s -i http://localhost:5001/api/lobby-stats")
            child.expect([r"\$", r"#", pexpect.EOF, pexpect.TIMEOUT], timeout=10)
