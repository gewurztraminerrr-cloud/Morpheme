...
- **Simultaneous Thumb Ascent & Shrinking During Stream & User Scroll**:
  ...
  - If the user scrolls down through the first batch of words while loading, the thumb updates smoothly in real time without locking, jumping, or clipping.
  - Keeps active DOM node count strictly memory-bounded (~400 nodes), completely preventing mobile GPU texture compositor exhaustion, zero black screen, and 60fps responsiveness across all phones, tablets, laptops, and desktops.
...
- **Uncapped Full Lexicon Streaming**:
  ...
  - Animates the live progress counter smoothly across the entire word count.
