...
* **Branch**: `main`
* **Date**: August 27, 2026
* **Latest Commit ID**: `3bc6dbfb15ba5beffb650742f5ebcf6f26477e23` (`3bc6dbf`)
...
### A. Unscramble Session History Scrollbar Thumb Movement Fix (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
- **Resolved Static Thumb Locking**:
  - Identified and removed `top: 0 !important;` from the `#unscramble-history-scrollbar-thumb` CSS rule in `play.css`, which was overriding all runtime JavaScript position updates and permanently locking the thumb at `0px`.
  ...
  - As users scroll or swipe through Session History, the glowing cyan thumb now glides smoothly in exact proportion to the scroll offset.

### B. Mobile Touch Responsiveness & Scrolling for Unscramble Session History (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### C. Elimination of Word Pill Vertical Clipping in Unscramble Session History (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
- **Complete Word Pill Visibility**:
  ...
  - Completely resolved the issue shown in `cutoff.jpg` where the bottom half of word pills and letters (`PUCKING`, `PLAICES`, `TUNINGS`, etc.) were cut off by card boundaries.

### D. Unscramble Session History Single-Row Mobile Header Layout (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### E. Unscramble Session History Custom Draggable Scroller & Thumb (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### F. Unscramble Session History "1/1 Found" Formatting & Layout (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### G. Steady Progressive Word List Streaming in "View Full List" (`static/js/tools.js`, `templates/index.html`)
- **Steady Background Word Loading**:
  - Implemented `startSteadyLoader()` and `stopSteadyLoader()` using animation frame pacing (`requestAnimationFrame`) to progressively stream and append chunks of 500 words smoothly into the DOM.
  - The initial batch renders instantly in $0\text{ms}$ while words steadily populate in the background without freezing the UI or dropping frames.
  - Displays a live progressive counter (`Loading… 15,500 / 199,429 words`) that updates dynamically until 100% of the lexicon is loaded.

### H. Mobile Tap Highlight & Blue Flash Removal on "View Full List" (`static/css/play.css`, `templates/index.html`)
...
### I. Lists Tool Header Cleanup & "View Full List" Button Placement (`templates/index.html`, `static/js/tools.js`)
...
### J. FAQ AW Dictionary Description Refinement (`templates/index.html`)
...
### K. Deduplication of 16+ Words Between AW & 16+ List (`dictionaries/added_words.txt`, `dictionaries/16plus.txt`)
...
### L. Instant Zero-Latency Room Loading & Synchronous Board Readiness (`app.py`, `static/css/style.css`, `static/js/app.js`, `static/js/lobby.js`, `static/js/play.js`, `templates/index.html`)
...
### M. Performance Efficiency (PE) Calibration & Dominant Score Trophy Awards (`game_room.py`, `app.py`, `static/js/play.js`, `templates/index.html`)
...
### N. Mobile Profile Search Input & Button Height Alignment (`static/css/play.css`, `static/css/style.css`, `templates/index.html`)
...
### O. Level Alignment for Rotate & Transpose Buttons on Mobile (`static/css/play.css`, `templates/index.html`)
...
### P. Cross-Browser Lobby Music on "ENTER LOBBY" Gateway (`static/js/app.js`, `templates/index.html`)
...
### Q. Desktop Color Chart Repositioning & Downward Mini-Popups (`static/css/style.css`, `templates/index.html`)
...
### R. Combo Checker Virtualized Column Rendering & Infinite Scrolling (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### S. Combo Checker Textbox & Button Height Sizing on Mobile (`static/css/play.css`, `templates/index.html`)
...
### T. Windowed Chunk Rendering & Seamless Mobile Word Jump in Tools Full List (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### U. Smooth Sliding Transitions on Bottom Navigation Buttons (`static/js/play.js`, `templates/index.html`)
...
### V. Mobile Intermission Bottom Navigation Bar in FCFS, SP, and CUBE Rooms (`static/css/play.css`, `templates/index.html`)
...
### W. Profile and Tools Controls Centering & Equal Panel Padding (`static/css/play.css`, `templates/index.html`)
...
### X. Drop-Down Menu Label Simplification: "Added Words" to "AW" (`templates/index.html`)
...
### Y. Lobby Live Player Count Accuracy & Accumulative Room Sync (`app.py`, `game_room.py`, `static/js/lobby.js`, `templates/index.html`)
...
### Z. FAQ Entry for Game Room Window Navigation (`templates/index.html`)
