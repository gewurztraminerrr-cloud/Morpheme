...
* **Branch**: `main`
* **Date**: August 27, 2026
* **Latest Commit ID**: `80c3fdb3471363935d4219d32d2cccce8ea694e7` (`80c3fdb`)
...
### A. Persistent Virtual List Scrollbar Scaling & Stream Completion Fix (`static/js/tools.js`, `templates/index.html`)
- **Resolved Sudden Thumb Enlargement After Stream**:
  - Identified that upon stream completion, `resultsEl._updateCustomScrollbar()` previously recalculated thumb height based on the active DOM slice (~400 nodes, ~3,000px height) rather than the true full dictionary length (469,764 words).
  - Updated `updateThumb()` and `onDragMove()` in `initCustomScrollbarForElement()` to scale against `_fullListAllWords.length` whenever viewing `full-list-modal-results`.
  - The thumb permanently remains sleek and small (28px minimum height) after loading completes, accurately representing the massive lexicon size.
- **Continuous Virtual Drag & Scroll Mapping**:
  - Dragging the thumb calculates target virtual window offsets across the full 469,764-word space with instant sub-slice rendering and zero memory expansion.

### B. 5.0s Progressive Block-by-Block Loading Rate & `[Loaded] / [Total]` Counter (`static/js/tools.js`, `templates/index.html`)
- **Block-by-Block Loading Counter**:
  - Configured `startSteadyLoader()` over a 5.0-second progression window with distinct block increments (e.g., 5,000 words per block for AW, 2,500 for NWL/CSW).
...
### C. Combo Checker LIC Tables Restoration & Dynamic Calibration (`app.py`, `static/css/play.css`, `templates/index.html`)
...
### D. Full Lexicon Uncapped Streaming & In-Memory Caching for "View Full List" (`static/js/tools.js`, `templates/index.html`)
...
### E. Global Scope & Immediate Availability for "View Full List" Modal (`static/js/tools.js`, `templates/index.html`, `static/css/play.css`)
...
### F. Real-Time Dynamic Scrollbar Scaling & Thumb Positioning Across All Devices (`static/js/tools.js`, `templates/index.html`)
...
### G. Unscramble Session History Scrollbar Thumb Movement Fix (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### H. Mobile Touch Responsiveness & Scrolling for Unscramble Session History (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### I. Elimination of Word Pill Vertical Clipping in Unscramble Session History (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### J. Unscramble Session History Single-Row Mobile Header Layout (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### K. Unscramble Session History Custom Draggable Scroller & Thumb (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### L. Unscramble Session History "1/1 Found" Formatting & Layout (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### M. Mobile Tap Highlight & Blue Flash Removal on "View Full List" (`static/css/play.css`, `templates/index.html`)
...
### N. Lists Tool Header Cleanup & "View Full List" Button Placement (`templates/index.html`, `static/js/tools.js`)
...
### O. FAQ AW Dictionary Description Refinement (`templates/index.html`)
...
### P. Deduplication of 16+ Words Between AW & 16+ List (`dictionaries/added_words.txt`, `dictionaries/16plus.txt`)
...
### Q. Instant Zero-Latency Room Loading & Synchronous Board Readiness (`app.py`, `static/css/style.css`, `static/js/app.js`, `static/js/lobby.js`, `static/js/play.js`, `templates/index.html`)
...
### R. Performance Efficiency (PE) Calibration & Dominant Score Trophy Awards (`game_room.py`, `app.py`, `static/js/play.js`, `templates/index.html`)
...
### S. Mobile Profile Search Input & Button Height Alignment (`static/css/play.css`, `static/css/style.css`, `templates/index.html`)
...
### T. Level Alignment for Rotate & Transpose Buttons on Mobile (`static/css/play.css`, `templates/index.html`)
...
### U. Cross-Browser Lobby Music on "ENTER LOBBY" Gateway (`static/js/app.js`, `templates/index.html`)
...
### V. Desktop Color Chart Repositioning & Downward Mini-Popups (`static/css/style.css`, `templates/index.html`)
...
### W. Combo Checker Virtualized Column Rendering & Infinite Scrolling (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### X. Combo Checker Textbox & Button Height Sizing on Mobile (`static/css/play.css`, `templates/index.html`)
...
### Y. Smooth Sliding Transitions on Bottom Navigation Buttons (`static/js/play.js`, `templates/index.html`)
...
### Z. Mobile Intermission Bottom Navigation Bar in FCFS, SP, and CUBE Rooms (`static/css/play.css`, `templates/index.html`)
...
### AA. Profile and Tools Controls Centering & Equal Panel Padding (`static/css/play.css`, `templates/index.html`)
...
### AB. Drop-Down Menu Label Simplification: "Added Words" to "AW" (`templates/index.html`)
...
### AC. Lobby Live Player Count Accuracy & Accumulative Room Sync (`app.py`, `game_room.py`, `static/js/lobby.js`, `templates/index.html`)
...
### AD. FAQ Entry for Game Room Window Navigation (`templates/index.html`)
