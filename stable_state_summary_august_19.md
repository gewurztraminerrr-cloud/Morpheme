...
* **Branch**: `main`
* **Date**: August 19, 2026
* **Commit ID**: `71894faed369becd1bd43837b9d3f4ebd06bb2bf` (`71894fa`)
* **Asset Version**: `v=33067`

---

## 2. Key Features, Improvements & Fixes in This Stable State

### A. Dictionary Duplicate Backups & Clean Definition Formatting (`app.py`, `dictionaries/`)
- **Untouched Duplicate Backups**: Created identical backup files for the Added Words list (`dictionaries/added_words_backup.txt` - 509,954 words) and definitions (`dictionaries/Definitions_backup.txt` - 279,498 definitions) to guarantee safe rollback.
- **Removed Leading `(noun)`**: Removed `(noun)` / `(Noun)` from the start of definitions across the entire dictionary lookup and resolution pipeline. Noun entries now start cleanly with their direct definition or root reference (e.g. `a horseman, also CABALLERO [n -S]` or `APPLE, the firm round edible fruit of the apple tree`). All other language origins (`(Hawaiian)`, `(French)`) and non-noun tags (`(verb)`, `(adjective)`) remain preserved.

### B. 24h Room Score Sum 0-Score Exclusion (`app.py`, `static/js/play.js`)
- **Strict Score $\ge 1$ Filtering**: Players with an overall total score of 0 are completely excluded from the Score Sum table across all four 24h rooms (`24h_4x4`, `24h_4x6`, `24h_5x7`, and `24h_6x8`).
- **Live & Database Filter Alignment**: Backfill queries, SQL aggregation (`HAVING MAX(d.score_sum) > 0`), in-memory room scans, and frontend render logic only display and count players who have earned a score of 1 or greater.

### C. Guaranteed Session Expired Notice Suppression on $\ge$ 1 Hour Return (`templates/index.html`, `static/js/app.js`, `static/js/play.js`)
...
### D. In-Place Word Definition Popover across Tools (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### E. 170× C-Accelerated Morpheme Metric & High-Speed Combo Checker (`app.py`, `morpheme_metric.c`, `static/js/tools.js`)
...
### F. Accumulative Lobby Real-Time Auto-Polling & Live Count Synchronization (`static/js/lobby.js`, `static/js/app.js`, `templates/index.html`)
...
### G. Unscramble Tool Desktop & Laptop Full-Width Panel Expansion (`templates/index.html`, `static/css/play.css`)
...
### H. Store Tool Collins Official Scrabble Words 2–15 Word List Clarification (`templates/index.html`)
...
### I. Acer Laptop & Desktop Layout Optimization (`static/css/play.css`)
...
### K. Critical Deadlock Resolution (`game_room.py`)
...
### L. Dedicated Lobby Refresh Button & Clean Interaction Rules (`templates/index.html`, `static/js/lobby.js`)
...
### M. Open Rooms & Closed Rooms Lobby Tabs (`templates/index.html`, `static/js/lobby.js`, `static/css/lobby.css`)
...
### N. User Follow Spectator Mode Logic (`app.py`, `play.js`, `tools.js`)
...
### O. Forum Multi-Image Upload (4 Images Cap & macOS/Cross-Platform Reliability) (`app.py`, `static/js/forum.js`, `templates/index.html`)
...
### P. Realistic 3D "ENTER LOBBY" Button (`templates/index.html`, `static/css/style.css`, `static/js/app.js`)
- 3D physical socket housing with multi-tiered 12-layer extrusion box shadows and persistent flattened press-down state.

### Q. Lists Tool "View Full List" Anti-Copy & Exact Spot Return (`templates/index.html`, `static/css/play.css`, `static/js/tools.js`)
