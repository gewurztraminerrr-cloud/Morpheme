...
def main():
    ...
    nwl_trie = WordTrie(nwl_words, min_len=3)
    unique_trie = WordTrie(unique_words, min_len=3)
    bonus_pool = [w for w in nwl_words if len(w) == 5] # Consistent length 5
    ...
    while True:
        ...
        path = None
        while path is None:
            tb = [[None for _ in range(4)] for _ in range(4)]
            path = embed(tb, bonus_word)
        board = tb
        
        # 2. Fill Base Positions (r+c)%2 == 0
        for r in range(4):
            for c in range(4):
                if board[r][c] is None and (r+c)%2 == 0:
                    board[r][c] = random.choices(ALPHABET, weights=LETTER_FREQ_USER, k=1)[0]
        
        # 3. Fill IO Positions (r+c)%2 == 1 with Greedy Logic
        ...
        for r, c in io_positions:
            ...
            for char in pool:
                board[r][c] = char
                found = unique_trie.solve(board)
                if len(found) > max_unique:
                    max_unique = len(found)
                    best_char = char
            board[r][c] = best_char
            
        # Cleanup any remaining None (if bonus word left some gaps)
        for r in range(4):
            for c in range(4):
                if board[r][c] is None: board[r][c] = random.choices(ALPHABET, weights=LETTER_FREQ_USER, k=1)[0]
        ...
        # Criteria: 50-100 words, "hard" (looking for high uniqueness)
        if 50 <= word_count <= 100 and unique_ratio >= 0.3: # Targeting 30%+ for "Hard" (3LM/4x4)
            duration = time.time() - start_time
            print(f"RESULT: Bonus={bonus_word}, Words={word_count}, Unique={unique_ratio:.1%}, Time={duration:.2f}s")
        ...
        if board_count % 500 == 0:
            print(f"Board #{board_count}: {word_count} words, {unique_ratio:.1%} unique")
