...
- **Tools, Mods, and Settings Mobile Back Navigation**: Added persistent fixed-position bottom navigation bars (`.mobile-bottom-nav`) containing `← Back` buttons (`#tools-mobile-back-btn`, `#mods-mobile-back-btn`, `#settings-mobile-back-btn`) inside `.tools-content`. When tapped on mobile devices/tablets, they smoothly slide the horizontal carousel back over to the menu tabs sidebar (`scrollTo({ left: 0, behavior: 'smooth' })`). On desktops and laptops (`@media (min-width: 901px)`), these buttons are hidden because the side menus remain visible beside the content.
- **Forum Category Back Navigation (Mobile)**: In `#forum-view-list`, added a fixed-position bottom navigation bar (`.forum-bottom-bar.mobile-only-bottom-bar`) with `← Back` (`#forum-category-back-btn`). Tapping it on mobile slides the user back to the categories sidebar (`.forum-sidebar`). Hidden on desktop/laptop.
- **Forum Thread & Create Post Bottom Navigation (Desktop & Mobile)**: Relocated `← Back to category` (`#forum-back-to-list`) in single post view (`#forum-view-post`) and `← Cancel` (`#forum-cancel-create`) in create post view (`#forum-view-create`) from the top sticky header to a clean, fixed-position bottom bar (`.forum-bottom-bar`) with backdrop blur and full edge-to-edge styling, active across all viewports.
- **Scroll Clearance**: Ensured tool panes, thread lists, and comments containers maintain `padding-bottom: 70px;` so content can be scrolled fully into view with ample clearance above the fixed bottom navigation bars.
