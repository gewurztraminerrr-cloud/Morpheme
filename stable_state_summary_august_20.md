...
* **Branch**: `main`
* **Date**: August 20, 2026
* **Commit ID**: `08a47041bf36289fa091778a765067995b347f7d` (`08a4704`)
* **Asset Version**: `v=33074`

---

## 2. Key Features, Improvements & Fixes in This Stable State

### A. Restored Clean Slow Gold Flash at 0:45 Intermission (`static/css/play.css`, `static/js/play.js`)
- **Smooth Gold Flash (No Pulsating/Scaling)**: Removed all transform/scaling and pulsating artifacts. When the 0:45 intermission mark is reached, the parameter labels cleanly snap to bright gold (`#ffd700`) with a gold text shadow, hold gold for 1 second, and smoothly fade back to normal text color over 4 seconds (`fadeGoldToNormal`).
...
### B. Safari Instant 0ms First-Paint Engine (`templates/index.html`, `static/js/app.js`, `app.py`)
...
### C. Lobby Filter Bar Organization (`templates/index.html`, `static/css/lobby.css`)
...
### D. Instant 24h Midnight Rollover & Elimination of Double Eviction (`game_room.py`, `static/js/play.js`)
...
### E. Automatic Root Word Definition Lookup & Bracket Appending (`app.py`)
...
### F. Clean Definition Formatting (Removed Leading `(noun)`) (`app.py`)
...
### G. Dictionary Cleanup: Obsolete Words, Abbreviations & Misspellings Removed (`dictionaries/`)
...
### H. 24h Room Score Sum 0-Score Exclusion (`app.py`, `static/js/play.js`)
...
### I. Guaranteed Session Expired Notice Suppression on $\ge$ 1 Hour Return (`templates/index.html`, `static/js/app.js`, `static/js/play.js`)
...
### J. In-Place Word Definition Popover across Tools (`static/js/tools.js`, `static/css/play.css`, `templates/index.html`)
...
### K. 170× C-Accelerated Morpheme Metric & High-Speed Combo Checker (`app.py`, `morpheme_metric.c`, `static/js/tools.js`)
...
### L. Accumulative Lobby Real-Time Auto-Polling & Live Count Synchronization (`static/js/lobby.js`)
...
### M. Unscramble Tool Desktop & Laptop Full-Width Panel Expansion (`templates/index.html`, `static/css/play.css`)
