...
* **Branch**: `main`
* **Date**: August 20, 2026
* **Commit ID**: `379f202a00df81e8eb17f938d613919e160e1d03` (`379f202`)
* **Asset Version**: `v=33082`

---

## 2. Key Features, Improvements & Fixes in This Stable State

### A. Accurate Lobby Player Counting & Accumulative Singleton Routing (`app.py`)
- **Deduplicated User Aggregation**: Updated `/api/lobby-stats` to aggregate unique human player IDs per game configuration (`set(user_id)`), guaranteeing that duplicate room instances or transient ghost states never inflate player counts (e.g. `[1]` will never show as `[2]`).
- **Canonical Accumulative Singletons**: Ensured that `/api/room/create` routes all Accumulative rooms into their canonical shared public singleton (`pub_v2_accumulative_{dims}_{time}`) so all players join the same active room.
...
### D. Instant 0ms Lobby User Counts Pre-Hydration & Gateway DOM Hierarchy (`templates/index.html`, `static/js/lobby.js`)
...
### R. Accumulative Lobby Real-Time Auto-Polling & Live Count Synchronization (`static/js/lobby.js`)
