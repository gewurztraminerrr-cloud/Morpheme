...
### Combo Checker Algorithm Rewrite & Rating Mid-Round Hardening (March 24, 2026 - Late Evening)
*   **Combo Checker Modernization**: Converted the `combos.java` utility into native Python inside `app.py`. Scrapped the flawed greedy index mapping penalty in favor of a genuine Longest Common Subsequence (LCS) edit distance check. Extremity mismatch letters are correctly ignored (0 cost), ensuring overlapping strings like `FERTILE`/`LITRE` and `WASTRIE`/`ASTRIDE` match perfectly at 0 and 1 MP respectively.
*   **Rating Integrity Hardening**: Fixed a critical leak allowing mid-round joiners to steal abandonment bounties. The server now comprehensively counts active players vs mid-round joiners vs UI quitters using `competitive_human_starters`. An entire unranked blackout engages automatically if a 1-player room finishes, and mid-round joiners are permanently banned from the `active_humans_remaining` and `soft_quitters` pools to ensure nobody's rating is artificially changed by late additions.
