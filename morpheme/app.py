...
def calculate_mp_pass(source, target, source_len, target_len):
    """Calculates MP score using strict moves + gaps formula."""
    # 1. Forward-Greedy Alignment
    ...
    # 4. Score Calculation (moves + source_gaps + target_gaps)
    mp = (count - count2) + (source_len - count) + (target_len - count)
    
    # 5. Hamming Optimization (for same-length words)
...
def check_and_add_mp(mp_groups, source_len, target_len, mp, word):
    """Strict JAR grouping thresholds."""
    if mp > 5 or word in mp_groups[mp]: return

    added = False
    if source_len == 3 or source_len == 4:
        if target_len >= source_len: added = True
    elif source_len == 5 or source_len == 6:
        if target_len >= 5 and mp <= 3:
            added = (target_len >= 6) if mp >= 3 else True
    elif source_len == 7 or source_len == 8:
        if target_len >= 5 and mp <= 4:
            added = (target_len >= 8) if (source_len == 7 and mp >= 4) else True
    elif source_len >= 9:
        if target_len >= 6 and mp <= 5:
            added = (target_len >= 8) if mp >= 5 else True
    
    if added:
        mp_groups[mp].append(word)

def check_and_add_lic(lic_groups, count, target_len, word):
    """Strict JAR LIC thresholds."""
    if count not in lic_groups: lic_groups[count] = []
    
    valid = False
    if count == 3 and target_len < 5: valid = True
    elif count == 4 and target_len < 6: valid = True
    ...
    elif count >= 8 and target_len < 9: valid = True
...
def tools_combo_check():
    ...
    if not search_term: return jsonify({'error': 'No search term provided'}), 400
        
    dictionary = load_tools_dictionary(dict_name)
    if not dictionary: return jsonify({'error': f'Dictionary {dict_name} not found'}), 404

    source_len = len(search_term)
    source_set = set(search_term)
    ...
    for word in dictionary:
        ...
        # Performance Pruning: Words must share at least 3 distinct letters
        if len(source_set & set(word)) < 3: continue
        
        # FAST PATH: Identical Match (0MP)
        if search_term == word or search_term_rev == word:
             check_and_add_mp(mp_groups, source_len, target_len, 0, word)
             check_and_add_lic(lic_groups, source_len, target_len, word)
             continue

        # 4-Pass Alignment
        # Pass 1 & 2 (Forward Target / Reversed Target)
        ...
        if word_rev != word:
            mp4, count4 = calculate_mp_pass(search_term_rev, word_rev, source_len, target_len)
            if mp4 <= 5: check_and_add_mp(mp_groups, source_len, target_len, mp4, word)
            check_and_add_lic(lic_groups, count4, target_len, word)

    # Convert/Sort and Return (Smallest First for LIC)
    for k in mp_groups: mp_groups[k].sort(key=lambda x: (len(x), x))
    for k in lic_groups:
        lic_groups[k] = list(set(lic_groups[k]))
        lic_groups[k].sort(key=lambda x: (len(x), x))
    
    return jsonify({'mp_groups': mp_groups, 'lic_groups': lic_groups})
