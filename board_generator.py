...
LETTER_FREQ_EASY = [
    ...
    500,  # I (Reduced per user request)
    1,    # J (Hardened)
    2,    # K (Hardened)
    560,  # L
...
LETTER_FREQ_SUPER_DENSITY = [
    ...
    150,  # G
    300,  # H
    700,  # I (Reduced per user request)
    1,    # J (Near Zero)
    2,    # K (Near Zero)
    500,  # L
    350,  # M
...
class BoardGenerator:
    ...
    def generate_board(
        ...
        # USER REQUEST: Use special Checkerboard IO and Base procedure for large boards unless pure Checkerboard is requested
        if rows * cols >= 35 and "checkerboard" not in str(board_format).lower():
