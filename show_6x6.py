...
def print_board(name, board, total, uniques, ratio, samples, bonus):
    ...
    for row in board:
        print("  " + " ".join(row))
    print(f"Sample Unique Words (min 6L): {', '.join(samples)}...")
...
if __name__ == '__main__':
    ...
    # Note: Using wider target range for 6x6/6L as it's harder to get exactly 50-100 6L words
    board1, _, _, _, _ = gen.generate_board('6x6', bonus1, (10, 100), 'NWL', 'Normal', min_len)
    t1, u1, r1, s1 = get_stats(board1, gen, min_len)
    print_board("Scenario 1 (Random + Bonus)", board1, t1, u1, r1, s1, bonus1)
    
    # --- Example 2: Scenario 2 ---
    bonus2 = random.choice(BONUS_POOL)
    board2, _, _, _, _ = gen.generate_board('6x6', bonus2, (10, 100), 'NWL', 'Normal', min_len)
    # Optimized once
    current = [row[:] for row in board2]
    r, c = random.randint(0, 5), random.randint(0, 5)
    _, start_u, _, _ = get_stats(current, gen, min_len)
    best_char = current[r][c]
    for char in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        current[r][c] = char
        _, uni, _, _ = get_stats(current, gen, min_len)
        if uni > start_u:
            start_u = uni
            best_char = char
    current[r][c] = best_char
    t2, u2, r2, s2 = get_stats(current, gen, min_len)
